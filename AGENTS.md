...
3. 本次任务对应的 skill（见下表）
4. 方法是 RD / DID / IV 时，再读 `references/methods/` 里对应的一页纸
5. 相关目录自己的 `README.md`

不要把上游仓库全文贴进规则，也不要安装需要付费密钥才能用的服务。外面仓库何时打开，只查 `references/UPSTREAM.md`。
...
识别上的实质决定，不确定就停，给 2–4 个选项，**不要替用户定**。至少包括：
...
可以默默做对、事后说一声的：设 seed、不碰 `data/raw/`、表由图由脚本出。

推断出来的假设写进 memo，标 `UNVERIFIED` 或 `[HUMAN: please confirm]`。  
假审稿（`referee-sim`）只出报告，不改用户的代码和正文。

方法细节只维护在 `references/methods/*.md`。和 skill 冲突时，停下来问用户，不要再去拼第三套上游说法。
...
审 RD / DID / IV 时：先打开对应方法纸，再写威胁和必做项。Stata 语法不够用，按 `references/UPSTREAM.md` 打开 stata-skill，不要把手册粘进本仓。
